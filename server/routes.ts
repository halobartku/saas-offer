  ...
  app.get("/api/templates", async (req, res) => {
    try {
      const allTemplates = await db
        .select({
          id: offerTemplates.id,
          name: offerTemplates.name,
          description: offerTemplates.description,
          createdAt: offerTemplates.createdAt,
          updatedAt: offerTemplates.updatedAt,
        })
        .from(offerTemplates);
    ...
    try {
      const items = await db
        .select({
          ...
          unitPrice: templateItems.unitPrice,
          discount: templateItems.discount,
          product: products
        })
        .from(templateItems)
        .leftJoin(products, eq(templateItems.productId, products.id))
        .where(eq(templateItems.templateId, req.params.id));
